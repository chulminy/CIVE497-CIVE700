...
% Dataset: Wine class dataset
% https://archive.ics.uci.edu/ml/datasets/Wine
% The dataset can be downloaded from the following link (data/wine.data)
% https://archive.ics.uci.edu/ml/machine-learning-databases/wine
% Kaggle: https://www.kaggle.com/brynja/wineuci
...
dataset = ReadWineData('data/wine.data');
net = set_nn; % you need to configure your network depending your dataset
test_nn(net, dataset);

function data = ReadWineData(file)
fid = fopen(file);
data_cell = textscan(fid,'%f %f %f %f %f %f %f %f %f %f %f %f %f %f', 178, 'Delimiter',',');
data = cell2mat(data_cell);
fclose(fid);
end

function net = set_nn
net.param.numepoch = 60; % number of epoch
...
[x, t] = data2xt_wine(dataset); % data transformation
...
function [x, t] = data2xt_wine(dataset)
